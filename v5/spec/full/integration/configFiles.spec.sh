@spec.config_files_are_searched_for_and_loaded_even_before_spec_main_invoked() {
  ...
  refute run ./specs-full.sh examples/specSpecs/customization/overridesMain/hello.spec.sh
  expect "$STDOUT" toContain "[hello.spec.sh]"

  # With spec.config.sh (via SPEC_CONFIG)
  export SPEC_CONFIG="examples/specSpecs/customization/overridesMain/spec.config.sh"
  
  assert run ./specs-full.sh examples/specSpecs/customization/overridesMain/hello.spec.sh
  expect "$STDERR" toBeEmpty
  expect "$STDOUT" not toContain "[hello.spec.sh]"
  expect "$STDOUT" toContain "Hi from customized main! You called me with examples/specSpecs/customization/overridesMain/hello.spec.sh"
}
